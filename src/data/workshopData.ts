      ...
      },
      {
        ...
        exercises: [
          ...
          },
          {
            ...
            durationMinutes: 40,
            description: "Reshape a raw payroll listing, where each employee's pay components run across the months, into a clean monthly table, then roll that up into the one-row-per-employee annual figures CP8D requires.",
            promptText: "STEP 1 — RESHAPE THE RAW PAYROLL FILE. Convert the file Client-Files/03_Payroll/DummyTrading_Payroll_Listing_2025.csv from its current nested horizontal format into a flat tabular report. Source structure: a header row of months (JAN to DEC); each employee starts with a row “Name: [Full Name]” then “I/C: [Number]”; component rows below (Salary Basic, Allowance, Overtime, EPF, Socso, PCB, etc.) with values spread across the month columns. Transformation: unpivot the 12 month columns into a single Month column; generate exactly 12 rows per employee repeating Name and I/C; turn pay components into columns in this exact order: Name, I/C, Month, Salary Basic, Allowance Transport, Allowance Proforma, Commission, Overtime, Angpau/Bonus, Income tax beared by employer, Deduction (Late), Others, Exempted allowance (petrol), EPF (employee), Socso (employee), EIS (employee), PCB, CP38, Zakat, EPF (employer), Socso (employer), EIS (employer). Formatting: clean I/C numbers to digits only, round all values to 2 decimal places, fill blanks with 0.00. Output as a downloadable CSV saved to the Outputs folder. STEP 2 — BUILD THE CP8D SUMMARY. Now roll that monthly table up into a Form E (CP8D) submission table with exactly one row per employee for the year, containing: Name, I/C, total Gross Remuneration (salary, allowances, commission, overtime and bonus combined), Benefits in Kind, total EPF (employee), total SOCSO (employee), total EIS (employee), total Zakat, total PCB and total CP38. Keep every employee who was paid at any point in the year, including mid-year joiners and leavers. Below the table, show the headcount and the column totals for Gross Remuneration, EPF (employee) and PCB so I can agree them to the payroll summary. Flag any employee whose 12 monthly rows do not add up to their annual total, and list anyone missing an I/C number. Save as DummyTrading_CP8D_2025_v1.csv in the Outputs folder.",
            promptExplanation: "Two-stage pattern: reshape the messy source into clean monthly records first, then aggregate to the annual per-employee figures CP8D needs. Splitting it lets you verify the reshape before the totals are built on top of it.",
            verificationSteps: [
              "Step 1 row count: Exactly 48 rows (4 employees × 12 months)",
              "Zero-fill check: Mid-year joiner (Lim Wei Ling) has Jan-Mar rows filled with 0.00",
              "Step 2 row count: One row per employee, headcount matches the payroll summary",
              "Cast test: One employee's annual Gross Remuneration equals the sum of their 12 monthly rows",
              "Control total: CP8D total PCB agrees to the sum of the monthly PCB column",
              "Mid-year joiner appears in CP8D with only their paid months included"
            ],
            goodToKnow: "LHDN revises the CP8D layout and its e-Filing upload format from time to time. Build the figures with this prompt, then map them onto the current year's official template rather than assuming the column order is fixed."
          }
        ]
      },
      {
